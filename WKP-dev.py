...
def load_lokatie():
    ...
    #extract for each waterbody the highest available version, use reset_index to prevent a multiindex dataframe
    lokaties=lokaties.groupby('waterlichaam_identificatie').max().reset_index()
    #inform user if verbose is TRUE
    if verbose:
...
def merge_data(dataframe,lokaties):
    ...
    lokaties[['waterlichaam_identificatie','stroomgebieddistrict_code',
